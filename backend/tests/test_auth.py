from datetime import datetime, timezone

import pytest
from fastapi.testclient import TestClient
from sqlalchemy.orm import Session

from app.core.config import settings
from app.core.database import SessionLocal
from app.core.security import hash_password
from app.main import app
from app.models.role import Role
from app.models.user import User


@pytest.fixture(autouse=True)
def setup_test_users():
    """Chuáº©n bá»‹ dá»¯ liá»‡u tÃ i khoáº£n test trÆ°á»›c má»—i bÃ i test."""
    db: Session = SessionLocal()
    try:
        # Äáº£m báº£o cÃ³ role driver vÃ  admin
        for r_name, r_desc in [("admin", "Admin Role"), ("driver", "Driver Role")]:
            if not db.query(Role).filter(Role.name == r_name).first():
                db.add(Role(name=r_name, description=r_desc))
        db.commit()

        # Táº¡o hoáº·c reset tÃ i khoáº£n test
        test_email = "test_driver@evcharging.vn"
        user = db.query(User).filter(User.email == test_email).first()
        if not user:
            role = db.query(Role).filter(Role.name == "driver").first()
            user = User(
                email=test_email,
                password_hash=hash_password("CorrectPass@123"),
                full_name="Driver Test",
                phone="0911222333",
                is_active=True,
                failed_login_count=0,
                locked_until=None,
                last_failed_ip=None,
            )
            if role:
                user.roles.append(role)
            db.add(user)
        else:
            user.password_hash = hash_password("CorrectPass@123")
            user.failed_login_count = 0
            user.locked_until = None
            user.last_failed_ip = None
            user.is_active = True
        db.commit()
    finally:
        db.close()


def test_login_success_sets_httponly_cookie():
    """AC 1: ÄÄƒng nháº­p Ä‘Ãºng thÃ´ng tin -> tráº£ vá» cookie httpOnly vÃ  user response."""
    with TestClient(app) as client:
        response = client.post(
            "/api/v1/auth/login",
            json={"email": "test_driver@evcharging.vn", "password": "CorrectPass@123"},
        )
        assert response.status_code == 200
        data = response.json()
        assert data["user"]["email"] == "test_driver@evcharging.vn"
        assert data["user"]["full_name"] == "Driver Test"
        assert "driver" in data["user"]["roles"]

        # Kiá»ƒm tra cookie trong response
        assert settings.session_cookie_name in response.cookies
        cookie = response.cookies[settings.session_cookie_name]
        assert cookie is not None


def test_login_error_message_identical_for_existing_and_non_existing_email():
    """AC 4: ThÃ´ng bÃ¡o lá»—i Ä‘Äƒng nháº­p sai email/máº­t kháº©u pháº£i Ä‘á»“ng nháº¥t Ä‘á»ƒ chá»‘ng enumeration."""
    with TestClient(app) as client:
        # Thá»­ vá»›i email khÃ´ng tá»“n táº¡i
        res_non_existing = client.post(
            "/api/v1/auth/login",
            json={"email": "non_existing_user_999@evcharging.vn", "password": "WrongPassword@123"},
        )
        # Thá»­ vá»›i email tá»“n táº¡i nhÆ°ng sai máº­t kháº©u
        res_existing_wrong_pw = client.post(
            "/api/v1/auth/login",
            json={"email": "test_driver@evcharging.vn", "password": "WrongPassword@123"},
        )

        assert res_non_existing.status_code == 401
        assert res_existing_wrong_pw.status_code == 401
        assert res_non_existing.json()["detail"] == res_existing_wrong_pw.json()["detail"]
        assert res_non_existing.json()["detail"] == "Email hoặc mật khẩu không chính xác"


def test_account_lockout_after_5_failed_attempts():
    """AC 2 & 3: Thá»­ sai 5 láº§n rá»“i nháº­p Ä‘Ãºng Ä‘á»ƒ kiá»ƒm tra tÃ i khoáº£n bá»‹ khÃ³a 15 phÃºt."""
    test_email = "test_driver@evcharging.vn"

    with TestClient(app) as client:
        # Nháº­p sai 4 láº§n Ä‘áº§u -> tráº£ vá» 401
        for attempt in range(1, 5):
            res = client.post(
                "/api/v1/auth/login",
                json={"email": test_email, "password": "WrongPassword@123"},
            )
            assert res.status_code == 401
            assert res.json()["detail"] == "Email hoặc mật khẩu không chính xác"

        # Láº§n thá»© 5 nháº­p sai -> tráº£ vá» 403 vÃ  bá»‹ khÃ³a
        res_5 = client.post(
            "/api/v1/auth/login",
            json={"email": test_email, "password": "WrongPassword@123"},
        )
        assert res_5.status_code == 403
        assert res_5.status_code == 403 and "15" in res_5.json()["detail"]

        # Kiá»ƒm tra trá»±c tiáº¿p trong Database: locked_until pháº£i cÃ³ giÃ¡ trá»‹ vÃ  > now
        db: Session = SessionLocal()
        try:
            user = db.query(User).filter(User.email == test_email).first()
            assert user is not None
            assert user.failed_login_count == 5
            assert user.locked_until is not None
            locked_dt = user.locked_until.replace(tzinfo=timezone.utc) if user.locked_until.tzinfo is None else user.locked_until; assert locked_dt > datetime.now(timezone.utc)
        finally:
            db.close()

        # Thá»­ nháº­p ÄÃšNG máº­t kháº©u khi Ä‘ang bá»‹ khÃ³a -> váº«n bá»‹ tá»« chá»‘i 403
        res_correct_while_locked = client.post(
            "/api/v1/auth/login",
            json={"email": test_email, "password": "CorrectPass@123"},
        )
        assert res_correct_while_locked.status_code == 403
        assert res_correct_while_locked.status_code == 403 and len(res_correct_while_locked.json()["detail"]) > 0


def test_get_current_user_me_and_logout():
    """Kiá»ƒm tra endpoint /me vá»›i cookie phiÃªn vÃ  endpoint /logout."""
    with TestClient(app) as client:
        # 1. Gá»i /me khi chÆ°a Ä‘Äƒng nháº­p -> 401
        unauth_res = client.get("/api/v1/auth/me")
        assert unauth_res.status_code == 401

        # 2. ÄÄƒng nháº­p
        login_res = client.post(
            "/api/v1/auth/login",
            json={"email": "test_driver@evcharging.vn", "password": "CorrectPass@123"},
        )
        assert login_res.status_code == 200

        # 3. Gá»i /me khi Ä‘Ã£ cÃ³ cookie trong phiÃªn -> 200
        me_res = client.get("/api/v1/auth/me")
        assert me_res.status_code == 200
        assert me_res.json()["email"] == "test_driver@evcharging.vn"

        # 4. ÄÄƒng xuáº¥t -> xÃ³a cookie
        logout_res = client.post("/api/v1/auth/logout")
        assert logout_res.status_code == 200

