...
> **Cập nhật lần cuối:** 2026-09-24 (FE phụ trách: `dtc245090028` — task Jira FE-Login + FE-Simulator).
...
| `backend/app/main.py` | FastAPI app — endpoint `/` health check |
...
| `backend/app/api/v1/endpoints/auth.py` | 05 | `POST /api/auth/login`, `/api/auth/me`, `/api/auth/ws-ticket` |
...
| `frontend/vite.config.js` | Vite dev server port 5173 + Proxy `/api` & `/ws` → `localhost:8000` |
...
| `frontend/src/pages/LoginPage.jsx` | Form Đăng nhập — email + password (icon ẩn/hiện) + validate + Toast lỗi (401/423) |
