...
async function runE2E() {
  ...
  await page.waitForSelector('text=ĐĂNG NHẬP HỆ THỐNG');
  await take('01_login_screen.png', 'Màn hình đăng nhập & 1-Click Role Switcher');

  // 2. Role: OPERATOR (CPO)
  console.log('\n--- 2. ROLE: OPERATOR (CPO - operator_a) ---');
  // Click nút CPO quick demo
  await page.click('button:has-text("Đơn vị Vận hành (CPO)")');
  ...
  try {
    ...
    if (startBtn) {
      ...
      await page.waitForTimeout(4000); // Đợi 2 nhịp websocket telemetry
    }
  } catch (e) {
  ...
  await page.goto(`${BASE_URL}/login`);
  await page.waitForTimeout(1000);
  await page.click('button:has-text("Khách hàng (Lái xe điện)")');
  await page.waitForURL(`${BASE_URL}/`);
  await page.waitForTimeout(2000);
  ...
  await page.goto(`${BASE_URL}/login`);
  await page.waitForTimeout(1000);
  // Điền form đăng nhập với driver_debt
  ...
  await page.goto(`${BASE_URL}/login`);
  await page.waitForTimeout(1000);
  await page.click('button:has-text("Quản trị viên (Admin)")');
  await page.waitForURL(`${BASE_URL}/`);
  await page.waitForTimeout(2000);
  ...
  await browser.close();
  console.log('\n=== HOÀN TẤT KIỂM THỬ E2E & CHỤP TOÀN BỘ ẢNH MÀN HÌNH THÀNH CÔNG ===');
