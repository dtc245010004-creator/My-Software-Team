...
export default function StationManagement() {
  ...
  const renderStatusBadge = (status) => {
    switch (status) {
      ...
      case 'MAINTENANCE':
        return (
          <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold bg-amber-50 text-amber-700 border border-amber-200 whitespace-nowrap">
  ...
  const renderRoleBadge = (userRole) => {
    switch (userRole) {
      case 'ADMIN':
        return <span className="px-2.5 py-1 rounded-lg text-xs font-black bg-purple-100 text-purple-800 border border-purple-200 whitespace-nowrap">SYSTEM ADMIN</span>;
      case 'CPO':
        return <span className="px-2.5 py-1 rounded-lg text-xs font-black bg-emerald-100 text-emerald-800 border border-emerald-200 whitespace-nowrap">CPO OPERATOR</span>;
      default:
        return <span className="px-2.5 py-1 rounded-lg text-xs font-black bg-blue-100 text-blue-800 border border-blue-200 whitespace-nowrap">EV DRIVER</span>;
    }
  };
      ...
      <div className="flex-1 flex flex-col overflow-y-auto">
        ...
        {activeTab === 'STATIONS' ? (
          <main className="p-6 md:p-8 space-y-6 w-full">
            ...
            {/* THẺ KHUNG KPI THỐNG KÊ (ĐÃ FIX SỰ CỐ BỊ NÉN KHUNG) */}
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
              <div className="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-sm min-h-[100px] flex flex-col justify-between">
              ...
              <div className="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-sm border-l-4 border-l-emerald-500 min-h-[100px] flex flex-col justify-between">
              ...
              <div className="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-sm border-l-4 border-l-blue-500 min-h-[100px] flex flex-col justify-between">
              ...
              <div className="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-sm border-l-4 border-l-amber-500 min-h-[100px] flex flex-col justify-between">
            ...
            <div className="bg-white rounded-2xl border border-slate-200/80 shadow-sm overflow-hidden">
              ...
              {/* BẢNG TRẠM SẠC (ĐÃ FIX CHỐNG XUỐNG DÒNG VÀ ĐÈ CHỮ "SỐ CỔNG") */}
                ...
                <table className="w-full text-left border-collapse min-w-[750px]">
                  <thead>
                    <tr className="bg-slate-100/70 border-b border-slate-200 text-[11px] font-black uppercase tracking-wider text-slate-600">
                      <th className="py-4 px-6 whitespace-nowrap">Mã Trụ (Unique Code)</th>
                      <th className="py-4 px-6">Tên Trụ Sạc & Vị Trí</th>
                      <th className="py-4 px-6">Loại Cổng & Công Suất</th>
                      <th className="py-4 px-6 text-center whitespace-nowrap">Số Cổng</th>
                    ...
                    {filteredStations.map((station) => (
                      <tr key={station.id} className="hover:bg-slate-50/80 transition-colors">
                        ...
                        <td className="py-4 px-6 text-center whitespace-nowrap">
                          <span className="inline-block font-extrabold text-slate-800 bg-slate-100 px-3 py-1 rounded-lg border border-slate-200">
          ...
          <main className="p-6 md:p-8 space-y-6 w-full">
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
              ...
              <div className="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-sm border-l-4 border-l-purple-500 min-h-[100px] flex flex-col justify-between">
                ...
                <table className="w-full text-left border-collapse min-w-[700px]">
                  <thead>
                    <tr className="bg-slate-100/70 border-b border-slate-200 text-[11px] font-black uppercase text-slate-600">
                      <th className="py-4 px-6 whitespace-nowrap">Mã ND</th>
                      <th className="py-4 px-6">Tên Người Dùng</th>
                      <th className="py-4 px-6">Email Đăng Nhập</th>
                      <th className="py-4 px-6 text-center whitespace-nowrap">Vai Trò (Role)</th>
                      <th className="py-4 px-6 text-center whitespace-nowrap">Trạng Thái</th>
                      <th className="py-4 px-6 text-right whitespace-nowrap">Thao Tác RBAC</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-100 text-xs">
                    {users.map((u) => (
                      <tr key={u.id} className="hover:bg-slate-50/80 transition-colors">
                        <td className="py-4 px-6 font-mono font-bold text-slate-900 whitespace-nowrap">{u.id}</td>
                        ...
                        <td className="py-4 px-6 text-center whitespace-nowrap">{renderRoleBadge(u.role)}</td>
                        <td className="py-4 px-6 text-center whitespace-nowrap">
